...
parser.add_argument("--n_class", type=int, default=22, help="number of classes")
parser.add_argument("--model", type=str, default='ast', help="the model used")
parser.add_argument("--dataset", type=str, default="audioset", help="the dataset used", choices=["Emergency", "audioset", "esc50", "speechcommands"])
...
if args.model == 'ast':
    ...
    norm_stats = {'Emergency':[-2.2162132, 4.137818], 'audioset':[-4.2677393, 4.5689974], 'esc50':[-6.6268077, 5.358466], 'speechcommands':[-6.845978, 5.5654526]}
    target_length = {'Emergency':1024, 'audioset':1024, 'esc50':512, 'speechcommands':128}
    # if add noise for data augmentation, only use for speech commands
    noise = {'Emergency': True, 'audioset': False, 'esc50': False, 'speechcommands':True}
    ...
    audio_conf = {'num_mel_bins': 128, 'target_length': target_length[args.dataset], 'freqm': args.freqm, 'timem': args.timem, 'mixup': args.mixup, 'dataset': args.dataset, 'mode':'train', 'mean':norm_stats[args.dataset][0], 'std':norm_stats[args.dataset][1],
                  'noise':noise[args.dataset], 'skip_norm':False}
    ...
    # bal 하이퍼 파라미터가 기본 esc에선 None임
    else:
        print('balanced sampler is not used')
        # num_workers는 기본적으로 32
        train_loader = torch.utils.data.DataLoader(
            dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf),
    ...
    # ! 1. 오디오 모델 AST를 여기서 받아옴 input dim 128, label dim은 50(esc 기준)
...
# 3. 위에서 받아온 argument를 가져와서 학습에 적용
