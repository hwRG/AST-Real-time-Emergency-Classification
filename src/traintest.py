...
def train(audio_model, train_loader, test_loader, args):
    ...
    # ! 6. 훈련을 위한 파라미터 저장 (필수들만 뽑아내는데 esc에서는 모든 파라미터 다 쓰는 듯)
    ...
    # Emergency 파라미터 세팅        
    elif args.dataset == 'Emergency':
        print('scheduler for Emergency is used') # !!! 건드려야 함
        # !? learning rate 스케줄이 뭐지
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, list(range(5,26)), gamma=0.85)
        main_metrics = 'acc'
        loss_fn = nn.CrossEntropyLoss() # loss는 cross entropy
        warmup = False
    ...
    epoch += 1
    # for amp
    scaler = GradScaler() # ! 7. 일반 gradient scaler 사용
    ...
    while epoch < args.n_epochs + 1:
        ...
        # ! 9. train_loader에서 스펙트로그램, 레이블 가져옴
        ...
        torch.save(audio_model.state_dict(), exp_dir + '/models/audio_model_' + str(epoch) + '.pth')
        if best_epoch == epoch:
